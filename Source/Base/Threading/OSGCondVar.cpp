...
bool CondVarPosix::wait(const float seconds)
{
   bool result(true);
      ...
      if ( timeToWait < 0)
      {
         ...
         if ( retcode != 0 )
         {
            ...
            msg_stream << "Unexpected error in OSG::CondVarPosix::wait(): "
                       << error_str;
            throw std::exception(msg_stream.str());
         }
      }
      ...
      else
      {
         ...
         const OSG::UInt64 UsecsPerSec(1000000);
         ...
         OSG::UInt64 abs_secs(0);
         //const OSG::UInt64 left_over_usecs = timeToWait.usec() % UsecsPerSec;
         const OSG::UInt64 left_over_usecs = (timeToWait * 1000000) % UsecsPerSec;
         OSG::UInt64 abs_usecs = now.tv_usec + left_over_usecs;
         ...
         abs_secs += (now.tv_sec + timeToWait);

         abs_timeout.tv_sec = abs_secs;
         abs_timeout.tv_nsec = 1000 * (abs_usecs);
         OSG_ASSERT(abs_timeout.tv_nsec < OSG::Int64(UsecsPerSec * 1000) &&
...
bool WinThreadCondVarBase::wait(const float seconds)
{
  // Avoid race conditions.
