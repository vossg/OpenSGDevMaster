...
LODSetupGraphOpTransitPtr LODSetupGraphOp::create( )
{
...
void LODSetupGraphOp::addLODInfo(Int32       LOD, 
                                 Real32      Range, 
                                 std::string SearchName)
{
    ...
    std::vector<LODInfo>::iterator it    = _mLODs.begin();
    std::vector<LODInfo>::iterator itEnd = _mLODs.end  ();
    ...
    if(added == false) 
        _mLODs.push_back(newLOD);
}

LODSetupGraphOp::LODInfo::LODInfo(Int32       LOD, 
                                  Real32      Range, 
                                  std::string Tag  ) :  
    mLOD  (LOD  ),
    mRange(Range),
...
LODSetupGraphOp::LODInfo::LODInfo(void) :
    mLOD  (-1   ),
    mRange(-1.0f),
    mTag  (""   )
{
...
Real32 LODSetupGraphOp::getRange(Int32 LOD)
{
    ...
    {
        if(_mLODs[i].mLOD == LOD) 
            return _mLODs[i].mRange;
    }
    
    // LOD not found, return the range for the highest LOD
    if(_mLODs.size() > 0) 
        return _mLODs[0].mRange;
    else 
        return 0.0f;
}
...
Action::ResultE LODSetupGraphOp::traverseLeave(Node            * const node, 
                                               Action::ResultE         res ) 
{
    return res;
}
