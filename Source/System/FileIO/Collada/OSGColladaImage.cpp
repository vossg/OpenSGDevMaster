 ...
 */
std::string ColladaImage::fixFilepath(std::string szFilepath)
{   
    ...
    {
        szFilepath = szFilepath.substr(1);
    }
    ...
    for(Int32 i = 0; i < 14; i++)
    {
        for(;;)
        {
            pos = szFilepath.find(escaped[i], pos);
            ...
            {
                szFilepath = szFilepath.replace(pos, 3, replacements[i]);
            }
        }
    }
...
std::string ColladaImage::fixImageFilepath(std::string szImgPath)
{
