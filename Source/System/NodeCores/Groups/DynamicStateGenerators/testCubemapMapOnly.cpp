...
OSG::NodeTransitPtr setupAnim(OSG::Node *lightBeacon)
{
    ...
    OSG::NodeUnrecPtr animRoot = OSG::Node::create();

    animRoot->setCore(scene_trans);

    dlight->addChild(animRoot);
...
//    sceneTrN->addChild(pAnimRoot  );
    ...
    OSG::NodeUnrecPtr pAnimRoot = setupAnim(b1n);
