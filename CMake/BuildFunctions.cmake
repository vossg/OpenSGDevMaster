...
FUNCTION(OSG_SETUP_LIBRARY_BUILD PROJ_DEFINE)
    ...
    IF(${PROJECT_NAME}_DEP_DEFS) 
        GET_TARGET_PROPERTY(_OSGTMPPROP ${PROJECT_NAME} COMPILE_DEFINITIONS)

        IF(_OSGTMPPROP)
          SET(_OSGTMPPROP ${_OSGTMPPROP};${${PROJECT_NAME}_DEP_DEFS})
        ELSE()
          SET(_OSGTMPPROP ${${PROJECT_NAME}_DEP_DEFS})
        ENDIF()
        SET_TARGET_PROPERTIES(${PROJECT_NAME} PROPERTIES
                              COMPILE_DEFINITIONS "${_OSGTMPPROP}")
    ENDIF()
    ...
    IF(${PROJECT_NAME}_DEP_CPP_DEF)
        ...
        IF(_OSGTMPPROP)
          SET(_OSGTMPPROP ${_OSGTMPPROP};${${PROJECT_NAME}_DEP_CPP_DEF})
        ELSE()
          SET(_OSGTMPPROP ${${PROJECT_NAME}_DEP_CPP_DEF})
        ENDIF()
        SET_TARGET_PROPERTIES(${PROJECT_NAME} PROPERTIES
                              COMPILE_DEFINITIONS "${_OSGTMPPROP}")
...
FUNCTION(OSG_SETUP_TEST_BUILD)
    ...
    FOREACH(EXE_SRC ${${PROJECT_NAME}_TEST_SRC})
        ...
        IF(${PROJECT_NAME}_DEP_TEST_DEFS) 
          GET_TARGET_PROPERTY(_OSGTMPPROP ${EXE} COMPILE_DEFINITIONS)

          IF(_OSGTMPPROP)
            SET(_OSGTMPPROP ${_OSGTMPPROP};${${PROJECT_NAME}_DEP_TEST_DEFS})
          ELSE()
            SET(_OSGTMPPROP ${${PROJECT_NAME}_DEP_TEST_DEFS})
          ENDIF()
