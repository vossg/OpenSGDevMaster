...
// OSGGPUVolRTV0Base.cpp file.
// To modify it, please change the .fcd file (OSGGPUVolRTV0.fcd) and
...
void GPUVolRTV0::initMethod(InitPhase ePhase)
{
...
GPUVolRTV0::GPUVolRTV0(const GPUVolRTV0 &source) :
    Inherited      (source      ),
    bInitialized   (false       ),
    renderbuffer   (0           ), 
    framebuffer    (0           ),
...
void GPUVolRTV0::changed(ConstFieldMaskArg whichField, 
                            UInt32            origin,
                            BitVector         details)
{
...
void GPUVolRTV0::setStepsize(Real32 rVal)
{
    stepsize = rVal;
}

Real32 GPUVolRTV0::getStepsize(void)
{
    return stepsize;
}


void GPUVolRTV0::setToggleVisuals(bool bVal)
{
    toggle_visuals = bVal;
}

bool GPUVolRTV0::getToggleVisuals(void)
{
    return toggle_visuals;
}


Action::ResultE GPUVolRTV0::renderEnter(Action *pAction)
{
    return Action::Continue;
}

Action::ResultE GPUVolRTV0::renderLeave(Action *pAction)
{
    return Action::Continue;
}
...
void GPUVolRTV0::dump(      UInt32    ,
                         const BitVector ) const
{
    SLOG << "Dump GPUVolRTV0 NI" << std::endl;
}


// ok let's start things up 
void GPUVolRTV0::init(DrawEnv *pEnv)
{
...
void GPUVolRTV0::resize(int w, int h)
{
	if (h == 0) h = 1;
	glViewport(0, 0, w, h);
	glMatrixMode(GL_PROJECTION);
...
void GPUVolRTV0::create_volumetexture()
{
...
void GPUVolRTV0::check_program_status(GLuint uiProgram)
{
    GLint  iInfoLength;
...
void GPUVolRTV0::enable_renderbuffers()
{
	glBindFramebufferEXT (GL_FRAMEBUFFER_EXT, framebuffer);
	glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, renderbuffer);
}

void GPUVolRTV0::disable_renderbuffers()
{
	glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
}
...
bool GPUVolRTV0::readProgram(std::string  &szTarget,
                             std::istream &iStream)
{
#define BUFSIZE 200
...
void GPUVolRTV0::check_shader_status(GLuint uiShader)
{
    GLint iStatus = 0;
...
void GPUVolRTV0::drawQuads(float x, float y, float z)
{
...
void GPUVolRTV0::set_tex_param(GLuint texLocU, GLuint tex, GLuint prog, 
...
void GPUVolRTV0::reshape_ortho(int w, int h)
{
	if (h == 0) h = 1;
	glViewport(0, 0,w,h);
	glMatrixMode(GL_PROJECTION);
...
void GPUVolRTV0::draw_fullscreen_quad(void)
{
