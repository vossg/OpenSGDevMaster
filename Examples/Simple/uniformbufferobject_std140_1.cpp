...
//
// This example differs from the uniformbufferobject_std140.cpp example
// that it uses the SimpleSHLChunk instead of the ShaderProgramChunk in
// combination with the ShaderProgram class.
//
//
...
int main(int argc, char **argv)
{
    ...
    // Otherwise OpenSG will complain about objects being alive after shutdown.
    {
        ...
        //
        OSG::SimpleSHLChunkUnrecPtr prog_chunk = OSG::SimpleSHLChunk::create();

        prog_chunk->setVertexProgram(get_vp_program());
        prog_chunk->setFragmentProgram(get_fp_program());

        prog_chunk->addUniformBlock("Materials", 1);    // block binding point
        prog_chunk->addUniformBlock("Lights",    2);    // block binding point
