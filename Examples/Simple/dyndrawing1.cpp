// OpenSG example: dyndrawing1.cpp
//
// Description:
//   This example shows how to create a render setup that allows the rendering
//   of a complex scene in its first viewport and on request (key '1') switches
//   into a 'dynamic' mode that allows you to render additional scenery dynamically.
//
// Problem:
//   In some domains such as CAD it is possible that the render scene is very
//   complex so that it is not possible to reach any reasonable frame rate. 
//   However you still need to be able to draw dynamic content in such a scenery.
//   Imagine for instance that you must dynamically resposion some 100 lines of grafic.
//
// Solution:
//   Separate the static and the dynamic rendering and render the static scenery from
//   an image. This is of course limited to dynamic modifications that leave the static
//   scenenry rendered from one perspective. Still it is very useful.
//
// Implementation:
//   The complex scene is rendered into a FBO that is subsequentely given to a
//   FBOBackground used by the dynamic viewport, i.e. the viewport that should handle
//   dynamic graphic.
//
// User interface:
