...
// shader code.
//
// This example uses solely the std430 layout.
//
...
// storage test block with the same layout as in the OpenGL std430 
...
//
// The OpenGL std430 specification test block.
...
const std::size_t num_materials = 5000;                           // any number of materials
...
{
    //
    // Introduction to the std430 storage layout
    // =========================================
    //
    // When using the "std430" storage layout, structures will be laid out in
    ...
    //
    // When using the std430 storage layout, shader storage blocks will be laid out in 
    // buffer storage identically to uniform and shader storage blocks using the
    // std140 layout, except that the base alignment and stride of arrays of scalars 
    // and vectors in rule 4 and of structures in rule 9 are not rounded up a multiple 
    ...
    //      array element, according to rules (1), (2), and (3), and are not rounded
    //      up to the base alignment of a vec4. The array may have padding at the
    ...
    //      members, and are not rounded up to the base alignment of a vec4. The
    ...
    // In order to qualify a block to use the std430 layout, you need to add a
    ...
    //     };
    //
    // This std430 qualification only works for shader storage buffer objects.
    //
    ...
    // Four-component vectors (e.g., vec4)     type. However, this is true only when
    ...
    //
    // A single-structure definition or        Structure alignment is the same as the
    // an array of structures                  alignment for the biggest structure
    ...
    ao = align_offset( 8, bo); bo = ao;                         //   struct {         //  9       8    28   32    (align begin)
    ...
    ao = align_offset( 8, bo); bo = ao;                         //   } f;             //  9       8    48   48    (pad end)
    ao = align_offset( 4, bo); bo = ao + sizeof(OSG::Real32);   //   Real32 g;        //  1       4    48   48    48..51
    ao = align_offset( 4, bo); bo = ao + sizeof(OSG::Real32);   //   Real32 h[2];     //  4       4    52   52    52..55 (h[0])
    ao = align_offset( 4, bo); bo = ao + sizeof(OSG::Real32);   //                    //          4    56   56    56..59 (h[1])
    ao = align_offset( 4, bo); bo = ao;                         //                    //  4       4    60   60    (pad end of h)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //   Matrix2x3f i;    //5/4/3    16    60   64    64..75  (i, column 0)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16    76   80    80..91  (i, column 1)
    ao = align_offset(16, bo); bo = ao;                         //                    // 5/4     16    92   96    (pad end of i)
    ao = align_offset(16, bo); bo = ao;                         //   struct {         //10/9     16    96   96    (align begin)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3u);    //     Vec3u j;       //  3      16    96   96    96..107 (o[0].j)
    ao = align_offset( 8, bo); bo = ao + sizeof(OSG::Vec2f);    //     Vec2f k;       //  2       8   108  112    112..119 (o[0].k)
    ao = align_offset( 4, bo); bo = ao + sizeof(OSG::Real32);   //     Real32 l[2];   //  4       4   120  120    120..123 (o[0].l[0])
    ao = align_offset( 4, bo); bo = ao + sizeof(OSG::Real32);   //                    //          4   124  124    124..127 (o[0].l[1])
    ao = align_offset( 4, bo); bo = ao;                         //                    //  4       4   128  128    (pad end of o[0].l)
    ao = align_offset( 8, bo); bo = ao + sizeof(OSG::Vec2f);    //     Vec2f m;       //  2       8   128  128    128..135 (o[0].m)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //     Matrix3f n[2]; // 6/4     16   136  144    144..155 (o[0].n[0], column 0)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   156  160    160..171 (o[0].n[0], column 1)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   172  176    176..187 (o[0].n[0], column 2)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   188  192    192..203 (o[0].n[1], column 0)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   204  208    208..219 (o[0].n[1], column 1)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   220  224    224..235 (o[0].n[1], column 2)
    ao = align_offset(16, bo); bo = ao;                         //                    // 6/4     16   236  240    (pad end of o[0].n)
    ao = align_offset(16, bo); bo = ao;                         //                    //10/9     16   240  240    (pad end of o[0])
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3u);    //                    //  3      16   240  240    240..251 (o[1].j)
    ao = align_offset( 8, bo); bo = ao + sizeof(OSG::Vec2f);    //                    //  2       8   252  256    256..263 (o[1].k)
    ao = align_offset( 4, bo); bo = ao + sizeof(OSG::Real32);   //                    //  4       4   264  264    264..267 (o[1].l[0])
    ao = align_offset( 4, bo); bo = ao + sizeof(OSG::Real32);   //                    //          4   268  268    268..271 (o[1].l[1])
    ao = align_offset( 4, bo); bo = ao;                         //                    //  4       4   272  272    (pad end of o[1].l)
    ao = align_offset( 8, bo); bo = ao + sizeof(OSG::Vec2f);    //                    //  2       8   272  272    272..279 (o[1].m)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    // 6/4     16   280  288    288..299  o[1].n[0], column 0)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   300  304    304..315 (o[1].n[0], column 1)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   316  320    320..331 (o[1].n[0], column 2)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   332  336    336..347 (o[1].n[1], column 0)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   348  352    352..363 (o[1].n[1], column 1)
    ao = align_offset(16, bo); bo = ao + sizeof(OSG::Vec3f);    //                    //         16   364  368    368..379 (o[1].n[1], column 2)
    ao = align_offset(16, bo); bo = ao;                         //                    // 6/4     16   380  384    (pad end of o[1].n)
    ao = align_offset(16, bo); bo = ao;                         //                    //10/9     16   384  384    (pad end of o[1])
                                                                //   } o[2];
                                                                // };
    return ao;
}
...
std::string get_fp_program()
{
    std::string fp_program =
            ...
            "};\n"
            "\n"
            "layout (std430) buffer TestBlock\n"
            "{\n"
            "    float a;\n"
            "    vec2  b;\n"
            "    vec3  c;\n"
